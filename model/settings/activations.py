...
def make_activation_module(config: dict) -> Union[nn.Identity, Exponential, nn.ReLU, nn.Sigmoid, nn.Softplus]:
    ...
    Returns:
        A activation module for a given name.
    """
...
def make_activation_function(config: dict) -> Callable[[torch.Tensor], torch.Tensor]:
    """
    The default args of some activation functions have been binded.
    One only need to use it as an unary function.

    Args:
        config (dict): A dictionary of configuration.
    Returns:
        A activation function for a given name.
    """
    ...
    if string_case_insensitive(name, "None"):
        ...
        raise KeyError("Wrong activation function name: got `{}`.".format(name))
